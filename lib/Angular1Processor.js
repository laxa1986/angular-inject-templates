...
var Angular1Processor = extend(RegexpProcessor, {
    ...
    replaceMatch : function(fileContext, match, cb, onErr) {
        ...
        var caseSensitiveTemplatePath = templatePath;
        try {
            caseSensitiveTemplatePath = trueCase.trueCasePathSync(templatePath);            
        } catch (error) {        
        }
        fs.readFile(caseSensitiveTemplatePath, {encoding: this.config.templateEncoding}, function(err, templateContent) {
            if (err) {                
                onError('Can\'t read template file: "' + caseSensitiveTemplatePath + '". Error details: ' + err);
                return;
            }
            ...
            minimizer.parse(templateContent, function (err, minifiedContent) {
                if (err) {
                    onError('Error while minifying angular template "' + caseSensitiveTemplatePath + '". Error from "minimize" plugin: ' + err);
                    return;
                }
